    ...
    def download_novel(self, novel_id: int) -> str:
        try:
            ...
            chapter_list = sorted(
                chapters.items(),
                key=lambda x: int(
                    re.search(r"\d+", x[0]).group() if re.search(r"\d+", x[0]) else "0"
                ),
            )
            ...
            with tqdm(total=total_chapters, desc="下载进度") as pbar:
                with concurrent.futures.ThreadPoolExecutor(
                    ...
                    future_to_chapter = {
                        ...
                        for i, (title, chapter_id) in enumerate(chapter_list)
                    }
            ...
            verified_content = verify_and_fix_chapters(
                novel_id, name, chapters, novel_content, self
            )
                ...
                novel_data = {
                    "_meta": {
                        ...
                        "completed_chapters": len(
                            [c for c in verified_content.values() if c]
                        ),
...
downloader = NovelDownloaderWrapper(
    config=config,
    progress_callback=lambda current, total, desc="", chapter="": socketio.emit(
        "progress",
        {
            "current": current,
            "total": total,
...
class DownloadQueue:
    ...
    def add(self, novel_id):
        with self.lock:
            # 检查是否已经下载成或正在下载
            if (
                novel_id not in self.queue
                and novel_id not in self.downloading_ids
                and novel_id not in self.completed_ids
            ):
                self.queue.append(novel_id)
                logger.info(f"Added novel ID {novel_id} to download queue")
            else:
                logger.info(
                    f"Novel ID {novel_id} is already in queue, downloading, or completed"
                )
    ...
    try:
        if os.path.exists(CONFIG_FILE):
            with open(CONFIG_FILE, "r", encoding="UTF-8") as f:
                ...
                config.save_path = saved_config.get(
                    "save_path", DOWNLOADS_DIR
                )  # 使用默认的DOWNLOADS_DIR
                config.save_mode = SaveMode(
                    saved_config.get("save_mode", config.save_mode.value)
                )
...
def download_novel(novel_id):
    ...
    try:
        ...
        while True:
            with tqdm(total=total_chapters, desc="下载进度") as pbar:
                with concurrent.futures.ThreadPoolExecutor(
                    max_workers=config.xc
                ) as executor:
                    future_to_chapter = {
                        executor.submit(
                            downloader._download_chapter, title, chapter_id, {}
                        ): (title, chapter_id, i)
                        for i, (title, chapter_id) in enumerate(chapter_list)
                    ...
                    for future in concurrent.futures.as_completed(future_to_chapter):
                        ...
                        socketio.emit(
                            "progress",
                            {
                                ...
                                "percentage": round(
                                    (completed_chapters / total_chapters * 100), 2
                                ),
                                "chapter": title,
                            },
                        )
            ...
            if retry_count >= max_retries:
                logger.warning(
                    f"达到最大重试次数({max_retries})，仍有 {len(failed_chapters)} 个章节下载失败"
                )
                break

            # 否则重试失败的章节
            logger.info(
                f"第 {retry_count} 次重试，重试 {len(failed_chapters)} 个失败的章节"
            )
            ...
            for i, (title, content) in enumerate(results):
                if title and content:
                    ...
                    chapter = epub.EpubHtml(
                        title=title, file_name=f"chap_{i+1}.xhtml", lang="zh"
                    )
...
def read_chapter(novel_id, chapter_title):
    ...
    try:
        logger.info(
            f"Attempting to read chapter: {chapter_title} from novel: {novel_id}"
        )
        ...
        json_path = os.path.join(
            BOOKSTORE_DIR, f"{novel_id}_{safe_name}.json"
        )  # 使用带ID的文件名
        ...
        if not os.path.exists(json_path):
            # 检查是否已经在下载
            if (
                novel_id not in download_queue.downloading_ids
                and novel_id not in download_queue.queue
            ):
                logger.info(
                    f"Novel file not found, adding to download queue: {novel_id}"
                )
                download_queue.add(novel_id)
                socketio.emit("queue_update", download_queue.get_status())
            ...
            # 等待下载完成
            while (
                novel_id in download_queue.downloading_ids
                or novel_id in download_queue.queue
            ):
                time.sleep(0.5)
        ...
        try:
            with open(json_path, "r", encoding="UTF-8") as f:
                ...
                return jsonify({"title": chapter_title, "content": chapter_content})
...
def handle_error(error):
    ...
    logger.exception("Full traceback:")
    return jsonify({"error": str(error), "details": traceback.format_exc()}), 500
...
def sort_chapter_list(chapter_list):
    ...
    def extract_chapter_number(title):
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                return (
                    cn2num(num)
                    if any(c in num for c in "一二三四五六七八九十百千万")
                    else float("inf")
                )
        return float("inf")
    ...
    return sorted(
        chapter_list,
        key=lambda x: (extract_chapter_number(x[0]), chapter_list.index(x)),
    )
...
def verify_and_fix_chapters(
    novel_id: str, name: str, chapters: dict, novel_content: dict, downloader
) -> dict:
    """验证章节完整性并修复缺失或损坏的章节"""
    ...
    if failed_chapters:
        ...
        while failed_chapters and retry_count < max_retries:
            ...
            if failed_chapters:
                logger.warning(
                    f"仍有 {len(failed_chapters)} 个章节修复失败，等待后重试"
                )
...
if __name__ == "__main__":
    ...
    # Run the server
    socketio.run(app, host="0.0.0.0", port=12930, debug=False, use_reloader=False)
