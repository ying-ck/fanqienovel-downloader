...
def down_zj(it):
    ...
    if response.status_code == 200:
        ...
        return [
            title_xpath_result[0],
            an,
            ele.xpath('//span[@class="info-label-yellow"]/text()'),
        ]
    else:
...
def down_text(it, mod=1):
    ...
    while True:
        try:
            ...
            n = "\n".join(
                ele.xpath('//div[@class="muye-reader-content noselect"]//p/text()')
            )
            if not n:
                n = "\n".join(
                    ele.xpath(
                        '//div[@class="muye-reader-content muye-reader-story-content noselect"]//p/text()'
                    )
                )
...
def down_text_old(it, mod=1):
    ...
    while True:
        try:
            res = req.get(
                "https://fanqienovel.com/api/reader/full?itemId=" + str(it),
                headers=headers2,
            )
        ...
        if config.get("enable_chapter_numbering", False):
            for index, chapter_title in enumerate(zj):
                ...
                text_file_path = os.path.join(
                    text_dir_path, sanitize_filename(new_chapter_title) + ".txt"
                )
                with open(text_file_path, "w", encoding="UTF-8") as text_file:
                    text_file.write(fg)
                    if config["kg"] == 0:
...
def down_book_epub(it, chapter_range=""):
    ...
    for chapter_title, _ in ordered_chapters:
        if chapter_title in downloaded_chapters:
            ...
            # 创建章节
            c = epub.EpubHtml(
                title=chapter_title, file_name=f"chapter_{len(chapters)+1}.xhtml"
            )
...
"""
    if config.get("enable_chapter_numbering", False):
        ...
        for chapter_title in zj:
            toc_content += (
                f"<li><a href='{chapter_title}.html'>{chapter_title}</a></li>"
            )
    ...
    for chapter_title, chapter_id in zj.items():
        tasks.append(
            executor.submit(
                download_chapter_html, chapter_title, chapter_id, existing_json_content
            )
        )
    for future in concurrent.futures.as_completed(tasks):
        chapter_title = future.result()
...
def download_chapter_html(chapter_title, chapter_id, existing_json_content):
    ...
    if f:
        ...
        if len(zj) > list(zj.keys()).index(chapter_title) + 1:
            if config.get("enable_chapter_numbering", False):
                next_chapter_key = list(zj.keys())[
                    list(zj.keys()).index(chapter_title) + 1
                ]
...
"""
            with open(
                os.path.join(book_dir, f"{chapter_number}_{chapter_title}.html"),
                "w",
                encoding="UTF-8",
            ) as chapter_file:
...
"""
            with open(
                os.path.join(book_dir, f"{chapter_title}.html"), "w", encoding="UTF-8"
            ) as chapter_file:
                chapter_file.write(chapter_html_content)
    return chapter_title
...
"""
    ...
    for chapter_title, chapter_id in zj.items():
        tasks.append(
            executor.submit(
                download_chapter_latex, chapter_title, chapter_id, existing_json_content
            )
        )
    for future in concurrent.futures.as_completed(tasks):
        chapter_title = future.result()
...
def book2down(inp):
    ...
    try:
        ...
        chapter_range = (
            f"({start_chapter}~{end_chapter})" if start_chapter and end_chapter else ""
        )
...
def perform_backup():
    ...
    for item in os.listdir(source_folder_path):
        ...
        if os.path.isfile(source_item_path) and os.path.basename(__file__) != item:
            ...
            os.path.isdir(source_item_path)
            and os.path.basename(__file__) != item
            and item != "backup"
        ):
            shutil.copytree(source_item_path, target_item_path)
...
"""
    ...
    if inp == "1":
        ...
        if inp2 == "1":
            ...
            config["enable_chapter_numbering"] = (
                input("是否开启章节排序功能？（1/2）：").lower() == "1"
            )
        elif inp2 == "7":
        ...
        with open(record_path, "r", encoding="UTF-8") as f:
            records = json.load(f)
        with open(
            os.path.join(script_dir, "list.txt"), "w", encoding="UTF-8"
        ) as list_file:
            for index, book_id in enumerate(records):
                ...
                list_file.write(
                    f'{index + 1}.{name}；作者：{author_name or "未知作者"}；{status}；共{chapter_count}章\n'
                )
