...
def search():
    while True:
        ...
        for i, book in enumerate(books):
            print(
                f"{i + 1}. 名称：{str_interpreter(book['book_name'])} 作者：{str_interpreter(book['author'])} ID：{book['book_id']} 字数：{book['word_count']}")
...
''')
    ...
    if inp == '1':
        ...
        if inp2 == '1':
            ...
            print('请选择：1.保存为单个 txt 2.分章保存 3.保存为 epub')
            inp3 = input()
            if inp3 == '1':
                config['save_mode'] = 1
            elif inp3 == '2':
        ...
        # 使用默认文本编辑器打开 urls.txt 文件供用户编辑
        root = Tk()
        root.withdraw()  # 隐藏主窗口
        root.update()  # 更新 Tkinter 的事件循环，确保窗口被隐藏
